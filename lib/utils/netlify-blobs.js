...
export async function getStore(storeName) {
  // Détection correcte de l'environnement Netlify
  // En production sur Netlify, NODE_ENV === 'production' et NETLIFY n'existe pas
  // On utilise la présence de NETLIFY_BLOBS_CONTEXT ou l'absence de localStorage
  const isNetlify = typeof localStorage === 'undefined' || 
  ...
  console.log(`☁️ Production mode: ${storeName} (Netlify Blobs)`);
  try {

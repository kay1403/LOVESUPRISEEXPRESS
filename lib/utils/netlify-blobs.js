// lib/utils/netlify-blobs.js - VERSION FINALE STABLE
const { getStore: getNetlifyStore } = require('@netlify/blobs');

// Configuration - Ces variables doivent être définies dans Netlify Dashboard
...
async function getStore(storeName) {
  ...
  console.log(`🔍 SITE_ID: ${SITE_ID ? 'défini' : 'MANQUANT'}`);
  console.log(`🔍 NETLIFY_AUTH_TOKEN: ${process.env.NETLIFY_AUTH_TOKEN ? 'défini' : 'MANQUANT'}`);

  // Essayer d'abord sans token (auto-détection Netlify)
  try {
    console.log('🔄 Tentative auto-détection Netlify...');
    ...
    // Fallback: configuration manuelle avec token
    if (!process.env.NETLIFY_AUTH_TOKEN) {
      throw new Error('NETLIFY_AUTH_TOKEN non défini et auto-détection échouée');
    }
    
    console.log('🔄 Tentative configuration manuelle...');

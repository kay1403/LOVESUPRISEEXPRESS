...
export async function getStore(storeName) {
  // 🔴 CRITIQUE: En production sur Netlify, NODE_ENV = 'production'
  // On détecte via NETLIFY (variable d'environnement) ou l'absence de localStorage
  ...
  if (!token) {
    ...
    throw new Error('NETLIFY_AUTH_TOKEN required');
  }

  console.log(`☁️ Production mode: ${storeName} (Netlify Blobs avec token)`);
  ...
  try {
    ...
    // Test d'accès pour vérifier que le store existe

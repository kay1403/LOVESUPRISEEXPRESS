// lib/utils/netlify-blobs.js - Version avec cache et compatibilité multi-versions
...
// ✅ BUG 3 CORRIGÉ: Cache des stores pour éviter appels réseau inutiles
...
// ✅ BUG 4 CORRIGÉ: Compatible avec v6 (items) et v7 (blobs)
async function getCommandes() {
  ...
  console.log(`🔑 Entrées trouvées: ${entries.length}`, 'Structure:', Object.keys(result));

  const commandes = [];
  for (const entry of entries) {
    try {
      const key = entry.key ?? entry;  // fallback si c'est une string directe

// lib/utils/netlify-blobs.js - Version avec correction de store.list()
...
async function getCommandes() {
  ...
  // ✅ CORRECTION: destructurer pour obtenir blobs
  const { blobs } = await store.list();
  console.log(`🔑 Clés trouvées: ${blobs.length}`);

  const commandes = [];
  for (const blob of blobs) {
    try {
      // ✅ CORRECTION: utiliser blob.key
      const cmd = await store.get(blob.key);
      if (cmd) {
        const parsed = typeof cmd === 'string' ? JSON.parse(cmd) : cmd;
        commandes.push(parsed);
      }
    } catch (e) {
      console.error(`Erreur lecture commande ${blob.key}:`, e.message);
    }
  }
...
async function getAvis(status = null) {
  ...
  // ✅ CORRECTION: destructurer pour obtenir blobs
  const { blobs } = await store.list();
  console.log(`🔑 Clés avis: ${blobs.length}`);

  const avis = [];
  for (const blob of blobs) {
    try {
      // ✅ CORRECTION: utiliser blob.key
      const a = await store.get(blob.key);
      if (a) {
      ...
      console.error(`Erreur lecture avis ${blob.key}:`, e.message);
    }
  }
...
async function getPhotos(category = null) {
  ...
  const photos = [];
  for (const blob of blobs) {
    if (category === null || blob.key.startsWith(category)) {
      photos.push({ id: blob.key, url: `/functions/get-photo?id=${blob.key}` });
    }
  }
  return photos;
}

...
__export(whatsapp_exports, {
  formatCommandeMessage: () => formatCommandeMessage,
  formatConfirmationClient: () => formatConfirmationClient,
...
async function sendWhatsApp(phoneNumber, message) {
  ...
  if (!apiKey) {
    console.log("\u26A0\uFE0F Pas de cl\xE9 API CallMeBot");
    return false;
  }
...
function formatCommandeMessage(commande) {
  return `\u{1F195} NOUVELLE COMMANDE !
...
function formatConfirmationClient(commande) {
  return `\u2705 Merci ${commande.clientName} ! Votre demande a bien \xE9t\xE9 re\xE7ue. Nous vous r\xE9pondons sous 30 min.`;
}
var CALLMEBOT_API;
...
var { saveCommand: saveCommand2 } = (init_netlify_blobs(), __toCommonJS(netlify_blobs_exports));
var { sendWhatsApp: sendWhatsApp2, formatCommandeMessage: formatCommandeMessage2, formatConfirmationClient: formatConfirmationClient2 } = (init_whatsapp(), __toCommonJS(whatsapp_exports));
exports.handler = async (event) => {
  ...
  try {
    ...
    const savedCommande = await saveCommand2(commande);
    ...
    try {
      ...
      console.log("Google Sheets non configur\xE9");
    }
    const entreprisePhone = process.env.ENTREPRISE_WHATSAPP || "250799366007";
    await sendWhatsApp2(entreprisePhone, formatCommandeMessage2(savedCommande));
    await sendWhatsApp2(commande.clientPhone, formatConfirmationClient2(savedCommande));
    return {
      statusCode: 200,
      headers,

...
var require_verify_token = __commonJS({
  "lib/utils/verify-token.js"(exports2) {
    ...
    exports2.isAdmin = async (event) => {
      try {
        const authHeader = event.headers.authorization;
        if (!authHeader || !authHeader.startsWith("Bearer ")) return false;
        const token = authHeader.split(" ")[1];
        const decoded = await exports2.verifyToken(token);
...
var { moderateAvis: moderateAvis2 } = (init_netlify_blobs(), __toCommonJS(netlify_blobs_exports));
var { isAdmin } = require_verify_token();
exports.handler = async (event) => {
  ...
  if (!admin) {
    return {
      statusCode: 401,
      headers,
      body: JSON.stringify({ error: "Non autoris\xE9" })
    };
  }
  try {
    ...
    const updated = await moderateAvis2(id, status);
    return {
      statusCode: 200,
      headers,
...
*/
//# sourceMappingURL=moderate-testimonial.js.map

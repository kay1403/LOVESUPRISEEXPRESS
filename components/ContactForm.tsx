...
export default function ContactForm() {
    ...
    switch(stepToValidate) {
      case 1:
        if (!formData.clientName.trim()) {
          setStepError(t('contactForm.validation.nameRequired'))
          return false
        }
        if (!formData.clientPhone.trim()) {
          setStepError(t('contactForm.validation.phoneRequired'))
          return false
        }
        if (!isValidPhone(formData.clientPhone)) {
          setStepError(t('contactForm.validation.invalidPhone'))
          return false
        }
        if (!formData.clientEmail.trim()) {
          setStepError(t('contactForm.validation.emailRequired'))
          return false
        }
        if (!isValidEmail(formData.clientEmail)) {
          setStepError(t('contactForm.validation.invalidEmail'))
          return false
        }
        return true
      ...
      case 2:
        if (!formData.destName.trim()) {
          setStepError(t('contactForm.validation.recipientNameRequired'))
          return false
        }
        if (!formData.destAddress.trim()) {
          setStepError(t('contactForm.validation.addressRequired'))
          return false
        }
        return true
      ...
      case 3:
        if (!formData.eventDate) {
          setStepError(t('contactForm.validation.dateRequired'))
          return false
        }
        if (!formData.eventTime) {
          setStepError(t('contactForm.validation.timeRequired'))
          return false
        }
        if (!formData.eventLocation.trim()) {
          setStepError(t('contactForm.validation.locationRequired'))
          return false
        }
        return true
      ...
      case 4:
        ...
        if (!hasService && !hasPack && !hasBasket) {
          setStepError(t('contactForm.validation.selectService'))
          return false
        }
        return true
  ...
  const handleBudgetChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value < totalPrice && value !== 0) {
      setBudgetError(`${t('contactForm.validation.budgetRequired')} ${totalPrice.toLocaleString()} RWF`)
    } else {
      setBudgetError('')
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!hasValidSelection) { alert(t('contactForm.validation.selectService')); return }
    if (formData.budget > 0 && formData.budget < totalPrice) { alert(`${t('contactForm.validation.budgetRequired')} ${totalPrice.toLocaleString()} RWF`); return }
  ...
  // Vue de succès avec clé dynamique sur la langue
  if (isSubmitted) {
    return (
      <section id="contact" className="py-24 bg-white">
        <div className="container-custom">
          <motion.div
            ...
            className="max-w-2xl mx-auto text-center p-12 bg-green-50 rounded-2xl"
          >
  ...
  const ReviewContent = () => (
    <div ref={reviewRef} className="bg-white p-6 rounded-xl space-y-4" style={{ fontFamily: 'Arial, sans-serif' }}>
      <div className="text-center border-b pb-4"><h2 className="text-2xl font-bold text-primary">LoveExpress</h2><p className="text-gray-500 text-sm">{t('contactForm.review.title')}</p><p className="text-xs text-gray-400 mt-1">{t('contactForm.review.date')}: {new Date().toLocaleDateString()}</p></div>
      <div><h3 className="font-semibold text-dark border-l-4 border-primary pl-3 mb-3">{t('contactForm.review.clientInfo')}</h3><div className="grid grid-cols-2 gap-2 text-sm"><p><span className="text-gray-500">{t('contactForm.fields.fullName')}:</span> {formData.clientName || t('common.notSpecified')}</p><p><span className="text-gray-500">{t('contactForm.fields.phone')}:</span> {formData.clientPhone || t('common.notSpecified')}</p></div></div>
      <div><h3 className="font-semibold text-dark border-l-4 border-primary pl-3 mb-3">{t('contactForm.review.recipientInfo')}</h3><div className="grid grid-cols-2 gap-2 text-sm"><p><span className="text-gray-500">{t('contactForm.fields.recipientName')}:</span> {formData.destName || t('common.notSpecified')}</p><p><span className="text-gray-500">{t('contactForm.fields.deliveryAddress')}:</span> {formData.destAddress || t('common.notSpecified')}</p></div></div>
      <div><h3 className="font-semibold text-dark border-l-4 border-primary pl-3 mb-3">{t('contactForm.review.eventInfo')}</h3><div className="grid grid-cols-2 gap-2 text-sm"><p><span className="text-gray-500">{t('contactForm.fields.eventType')}:</span> {formData.eventType}</p><p><span className="text-gray-500">{t('contactForm.fields.eventDate')}:</span> {formData.eventDate || t('common.notSpecified')}</p></div></div>
      <div><h3 className="font-semibold text-dark border-l-4 border-primary pl-3 mb-3">{t('contactForm.review.services')}</h3>
        <ul className="text-sm space-y-1">
          {formData.selectedPacks.map((packId: number) => {
            const pack = packs.find((p: Pack) => p.id === packId)
            return <li key={packId}>• {pack?.name} : {pack?.price.toLocaleString()} RWF</li>
          })}
          {formData.selectedServices.includes(2) && <li>• {t('services.surprise.title')} : 200 000 RWF</li>}
          {formData.selectedServices.includes(3) && <li>• {t('services.custom.title')} : 35 000 RWF ({t('common.estimate')})</li>}
          {formData.selectedServices.includes(4) && <li>• {t('services.flower.title')} : 15 000 RWF</li>}
      ...
      <div><h3 className="font-semibold text-dark border-l-4 border-primary pl-3 mb-3">{t('contactForm.review.delivery')}</h3><p className="text-sm">{formData.deliveryMethod === 'delivery' ? t('contactForm.deliveryMethods.delivery') + ' (+5 000 RWF)' : t('contactForm.deliveryMethods.pickup')}</p></div>
      <div className="border-t pt-3"><div className="flex justify-between"><span className="font-semibold">{t('contactForm.review.total')} :</span><span className="font-bold text-primary text-lg">{totalPrice.toLocaleString()} RWF</span></div></div>
      {formData.message && (<div className="bg-primaryLight p-3 rounded-lg"><p className="text-sm italic">"{formData.message}"</p></div>)}
      <div className="text-center text-xs text-gray-400 pt-4 border-t"><p>LoveSurpriseExpress - {t('footer.tagline')}</p><p>Tel: +250 799 366 007</p></div>
    </div>
  )
      ...
      <div className="container-custom">
        ...
        >
          <div className="inline-flex items-center gap-2 bg-primary/10 px-4 py-2 rounded-full mb-4">
            ...
            <span className="text-xs font-medium text-primary uppercase tracking-wider">{t('contactForm.badge')}</span>
          </div>
        ...
        <div className="max-w-3xl mx-auto">
          {showMaintenance ? (
            <MaintenanceBanner 
              message={maintenance.message || t('maintenance.defaultMessage') || "Le formulaire de commande est temporairement désactivé pour maintenance."}
            ...
            <>
              <div className="mb-8">
                <div className="flex justify-between mb-2 text-sm text-gray-500">
                  <span>{t('contactForm.step')} {step} / 6</span>
                ...
                <AnimatePresence mode="wait">
                  {step === 1 && (
                    <motion.div key="step1" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} className="space-y-6">
                      <h3 className="text-2xl font-bold text-dark mb-6">{t('contactForm.steps.0')}</h3>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">{t('contactForm.fields.fullName')} <span className="text-red-500">*</span></label>
                        <input type="text" value={formData.clientName} onChange={(e) => setFormData({...formData, clientName: e.target.value})} required className="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-primary" placeholder={t('contactForm.placeholders.fullName')} />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">{t('contactForm.fields.phone')} <span className="text-red-500">*</span></label>
                        <input type="tel" value={formData.clientPhone} onChange={(e) => setFormData({...formData, clientPhone: e.target.value})} required className="w-full px-4 py-3 border rounded-lg" placeholder={t('contactForm.placeholders.phone')} />
                      </div>
                      <div>
                  ...
                  {step === 2 && (
                    <motion.div key="step2" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} className="space-y-6">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">{t('contactForm.fields.recipientName')} <span className="text-red-500">*</span></label>
                        <input type="text" value={formData.destName} onChange={(e) => setFormData({...formData, destName: e.target.value})} required className="w-full px-4 py-3 border rounded-lg" placeholder={t('contactForm.placeholders.recipientName')} />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">{t('contactForm.fields.recipientPhone')} <span className="text-gray-400 text-xs">({t('common.optional')})</span></label>
                        <input type="tel" value={formData.destPhone} onChange={(e) => setFormData({...formData, destPhone: e.target.value})} className="w-full px-4 py-3 border rounded-lg" placeholder={t('contactForm.placeholders.recipientPhone')} />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">{t('contactForm.fields.deliveryAddress')} <span className="text-red-500">*</span></label>
                        <input type="text" value={formData.destAddress} onChange={(e) => setFormData({...formData, destAddress: e.target.value})} required className="w-full px-4 py-3 border rounded-lg" placeholder={t('contactForm.placeholders.deliveryAddress')} />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">{t('contactForm.fields.age')} <span className="text-gray-400 text-xs">({t('common.optional')})</span></label>
                        <input type="text" value={formData.destAge} onChange={(e) => setFormData({...formData, destAge: e.target.value})} className="w-full px-4 py-3 border rounded-lg" placeholder={t('contactForm.placeholders.age')} />
                      </div>
                      <div className="flex gap-4"><button type="button" onClick={prevStep} className="btn-secondary flex-1">{t('contactForm.buttons.back')}</button><button type="button" onClick={nextStep} className="btn-primary flex-1">{t('contactForm.buttons.next')}</button></div>
                    </motion.div>
                  )}
                  ...
                  {step === 3 && (
                    <motion.div key="step3" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} className="space-y-6">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">{t('contactForm.fields.eventType')} <span className="text-red-500">*</span></label>
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">{t('contactForm.fields.eventDate')} <span className="text-red-500">*</span></label>
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">{t('contactForm.fields.eventTime')} <span className="text-red-500">*</span></label>
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">{t('contactForm.fields.eventLocation')} <span className="text-red-500">*</span></label>
                        <input type="text" value={formData.eventLocation} onChange={(e) => setFormData({...formData, eventLocation: e.target.value})} required className="w-full px-4 py-3 border rounded-lg" placeholder={t('contactForm.placeholders.eventLocation')} />
                      </div>
                      <div className="flex gap-4"><button type="button" onClick={prevStep} className="btn-secondary flex-1">{t('contactForm.buttons.back')}</button><button type="button" onClick={nextStep} className="btn-primary flex-1">{t('contactForm.buttons.next')}</button></div>
                    </motion.div>
                  )}
                  ...
                  {step === 4 && (
                    <motion.div key="step4" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} className="space-y-6">
                      <h3 className="text-2xl font-bold text-dark mb-6">{t('contactForm.steps.3')}</h3>
                      <div className="border rounded-xl p-4">
                        <div className="flex items-center gap-2 mb-3"><PartyPopper size={18} className="text-primary" /><h4 className="font-semibold text-dark">{t('services.party.title')}</h4></div>
                      ...
                      <label className={`flex items-center justify-between p-4 border rounded-xl cursor-pointer transition ${formData.selectedServices.includes(2) ? 'border-primary bg-primaryLight' : 'border-gray-200'}`}>
                        <div className="flex items-center gap-3"><Sparkles size={18} className="text-primary" /><div><span className="font-medium">{t('services.surprise.title')}</span><p className="text-xs text-gray-500">{t('services.surprise.description')}</p></div></div>
                      ...
                      <label className={`flex items-center justify-between p-4 border rounded-xl cursor-pointer transition ${formData.selectedServices.includes(3) ? 'border-primary bg-primaryLight' : 'border-gray-200'}`}>
                        <div className="flex items-center gap-3"><Globe size={18} className="text-primary" /><div><span className="font-medium">{t('services.custom.title')}</span><p className="text-xs text-gray-500">{t('services.custom.description')}</p></div></div>
                      ...
                      <label className={`flex items-center justify-between p-4 border rounded-xl cursor-pointer transition ${formData.selectedServices.includes(4) ? 'border-primary bg-primaryLight' : 'border-gray-200'}`}>
                        <div className="flex items-center gap-3"><Flower2 size={18} className="text-primary" /><div><span className="font-medium">{t('services.flower.title')}</span><p className="text-xs text-gray-500">{t('services.flower.description')}</p></div></div>
                      ...
                      <div className="border rounded-xl p-4">
                        <div className="flex items-center gap-2 mb-3"><Gift size={18} className="text-primary" /><h4 className="font-semibold text-dark">{t('giftbaskets.title')}</h4></div>
                              ...
                              {formData.selectedBaskets.some((b: { id: number }) => b.id === basket.id) && (
                                <div className="flex gap-3 ml-6 mt-2">
                                  <label className="flex items-center gap-2">
                                    ...
                                    <span className="text-sm">{t('giftbaskets.standard')}: {basket.standard.toLocaleString()} RWF</span>
                                  </label>
                                  <label className="flex items-center gap-2">
                                    ...
                                    <span className="text-sm">{t('giftbaskets.premium')}: {basket.premium.toLocaleString()} RWF</span>
                                  </label>
                                </div>
                              )}
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">{t('contactForm.fields.message')} <span className="text-gray-400 text-xs">({t('common.optional')})</span></label>
                        <textarea value={formData.message} onChange={(e) => setFormData({...formData, message: e.target.value})} rows={3} className="w-full px-4 py-3 border rounded-lg" placeholder={t('contactForm.placeholders.message')} />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">{t('contactForm.fields.specialInstructions')} <span className="text-gray-400 text-xs">({t('common.optional')})</span></label>
                        <textarea value={formData.specialInstructions} onChange={(e) => setFormData({...formData, specialInstructions: e.target.value})} rows={2} className="w-full px-4 py-3 border rounded-lg" placeholder={t('contactForm.placeholders.specialInstructions')} />
                      </div>
                      <div className="flex gap-4"><button type="button" onClick={prevStep} className="btn-secondary flex-1">{t('contactForm.buttons.back')}</button><button type="button" onClick={nextStep} className="btn-primary flex-1">{t('contactForm.buttons.next')}</button></div>
                    </motion.div>
                  )}
                  ...
                  {step === 5 && (
                    <motion.div key="step5" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} className="space-y-6">
                      <h3 className="text-2xl font-bold text-dark mb-6">{t('contactForm.steps.4')}</h3>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">{t('contactForm.fields.deliveryMethod')}</label>
                        <div className="grid grid-cols-2 gap-4">
                          <label className={`flex items-center gap-3 p-4 border rounded-lg cursor-pointer ${formData.deliveryMethod === 'delivery' ? 'border-primary bg-primaryLight' : 'border-gray-200'}`}>
                            ...
                            <div><span className="font-semibold block">{t('contactForm.deliveryMethods.delivery')}</span><span className="text-xs text-gray-500">+5 000 RWF</span></div>
                          </label>
                          <label className={`flex items-center gap-3 p-4 border rounded-lg cursor-pointer ${formData.deliveryMethod === 'pickup' ? 'border-primary bg-primaryLight' : 'border-gray-200'}`}>
                            ...
                            <div><span className="font-semibold block">{t('contactForm.deliveryMethods.pickup')}</span><span className="text-xs text-gray-500">{t('contactForm.deliveryMethods.pickupFree')}</span></div>
                          </label>
                        </div>
                      </div>
                      <div className="bg-primaryLight rounded-xl p-5">
                        <h4 className="font-semibold text-dark mb-3">{t('contactForm.review.services')}</h4>
                        <div className="space-y-2 text-sm">
                          ...
                          {formData.selectedServices.includes(2) && <div className="flex justify-between"><span>{t('services.surprise.title')}</span><span className="font-bold">200 000 RWF</span></div>}
                          {formData.selectedServices.includes(3) && <div className="flex justify-between"><span>{t('services.custom.title')}</span><span className="font-bold">~35 000 RWF</span></div>}
                          {formData.selectedServices.includes(4) && <div className="flex justify-between"><span>{t('services.flower.title')}</span><span className="font-bold">15 000 RWF</span></div>}
                          {formData.selectedBaskets.map((b: { id: number; version: 'standard' | 'premium' }) => {
                            const basket = STATIC_BASKETS.find((bk: StaticBasket) => bk.id === b.id)
                            const basketName = basketsDataFromCMS?.find((bk: any) => bk.id === b.id)?.name || basket?.nameKey
                          ...
                          {formData.deliveryMethod === 'delivery' && <div className="flex justify-between"><span>{t('contactForm.deliveryMethods.delivery')}</span><span className="font-bold">+5 000 RWF</span></div>}
                          <div className="border-t pt-2 mt-2 flex justify-between"><span className="font-bold">{t('contactForm.review.total')} :</span><span className="font-bold text-primary text-lg">{totalPrice.toLocaleString()} RWF</span></div>
                        </div>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">{t('contactForm.fields.budget')} (RWF) <span className="text-gray-400 text-xs">({t('common.optional')} - {t('contactForm.budgetHint')})</span></label>
                        ...
                        <p className="text-xs text-gray-400 mt-1">{t('contactForm.budgetSuggestion')} : {totalPrice.toLocaleString()} RWF</p>
                      </div>
                      <div className="flex gap-4"><button type="button" onClick={prevStep} className="btn-secondary flex-1">{t('contactForm.buttons.back')}</button><button type="button" onClick={nextStep} className="btn-primary flex-1">{t('contactForm.buttons.next')}</button></div>
                    </motion.div>
                  )}
                  ...
                  {step === 6 && (
                    <motion.div key="step6" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} className="space-y-6">
                      <h3 className="text-2xl font-bold text-dark mb-6">{t('contactForm.steps.5')}</h3>
                      <ReviewContent />
                      <label className="flex items-center gap-3 p-4 border rounded-lg cursor-pointer hover:bg-primaryLight">
                        ...
                        <span>{t('contactForm.options.discreet')} <span className="text-gray-400 text-xs">({t('common.optional')})</span></span>
                      </label>
                      <label className="flex items-center gap-3 p-4 border rounded-lg cursor-pointer hover:bg-primaryLight">
                        ...
                        <span>{t('contactForm.options.needsPersonPresent')} <span className="text-gray-400 text-xs">({t('common.optional')})</span></span>
                      </label>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">{t('contactForm.fields.additionalNotes')} <span className="text-gray-400 text-xs">({t('common.optional')})</span></label>
                        <textarea value={formData.additionalNotes} onChange={(e) => setFormData({...formData, additionalNotes: e.target.value})} rows={2} className="w-full px-4 py-3 border rounded-lg" placeholder={t('contactForm.placeholders.additionalNotes')} />
                      </div>
                      <div className="flex flex-col gap-3">
                        <div className="flex gap-4">
                          <button type="button" onClick={() => goToStep(1)} className="btn-secondary flex-1">{t('contactForm.buttons.modify')}</button>
                          <button type="button" onClick={downloadPDF} className="bg-gray-100 text-gray-700 px-4 py-3 rounded-full font-semibold hover:bg-gray-200 transition flex items-center justify-center gap-2 flex-1"><Download size={18} /> {t('contactForm.buttons.downloadPDF')}</button>
                        </div>
                        <button type="submit" disabled={isSubmitting || (!hasValidSelection) || (formData.budget > 0 && formData.budget < totalPrice)} className="btn-primary w-full">{isSubmitting ? t('common.loading') : t('contactForm.buttons.confirm')}</button>
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>

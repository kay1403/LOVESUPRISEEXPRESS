// Utilitaires pour Netlify Blobs (stockage gratuit)
// Documentation: https://docs.netlify.com/blobs/

export async function getStore(storeName) {
  // En développement, utiliser localStorage ou mémoire
  ...
  // En production, utiliser Netlify Blobs
...
export async function saveCommand(data) {
  ...
  const id = `CMD_${Date.now()}_${Math.random().toString(36).substr(2, 6)}`;
  const commande = {
    id,
    ...data,
    status: 'pending',
...
export async function saveAvis(avis) {
  ...
  const id = `AVIS_${Date.now()}_${Math.random().toString(36).substr(2, 6)}`;
  const nouvelAvis = {
    id,
    ...avis,
    status: 'pending',
...
export async function savePhoto(file, category) {
  ...
  const id = `${category}_${Date.now()}_${Math.random().toString(36).substr(2, 6)}`;
